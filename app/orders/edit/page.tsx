        ...
        <div className="max-w-3xl mx-auto space-y-6 pb-10">
            ...
            <div className="bg-white p-5 rounded-2xl shadow space-y-4">
                ...
                {items.map((item: any) => (
                    <div
                        key={item.id}
                        className={`flex gap-4 p-3 rounded-xl border border border-gray-300 rounded-xl transition ${item.status === "CANCELLED" ? "opacity-50 bg-gray-50" : ""
                            }`}
                    >
                        {/* Image */}
                        <img
                        ...
                        <div className="flex-1 flex flex-col justify-between">
                            <div>
                                ...
                                {/* Status */}
                                <span
                                    className={`inline-block mt-1 px-2 py-1 text-xs rounded-full font-medium ${getStatusClass(item.status)}`}
                                >
                                    {getStatusLabel(item.status)}
                                </span>

                                {/* Quantity */}
                                {item.status === "WAITING" || item.status === "NEW" ? (
                                ...
                                {item.status === "WAITING" || item.status === "NEW" ? (
                                    <textarea
                                        ...
                                        className="w-full mt-2 p-2 border border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-400 outline-none"
                                    />
                                ) : (
            ...
            <div className="bg-white p-5 rounded-2xl shadow space-y-2">
                <div className="flex justify-between text-lg font-bold">
                    <span>Tạm tính <p className="text-sm font-normal text-gray-500">(Chưa gồm thuế)</p></span>
